...
const TONE_DESCRIPTIONS: Record<Tone, string> = {
  ...
  confident:    'direct and assertive — lead with what you bring',
}

function generatePersonalizedOutreach(body: GenerateOutreachBody): string {
  const {
    yourName, background, targetRole, achievements, industry, location,
    companyStage, remotePreference, company, recruiterName,
    recruiterRole, recruiterLocation, jobDescription, tone = 'professional',
  } = body
  ...
  const contextLines = [
    ...
    jobDescription     ? `Job description excerpt: ${jobDescription.slice(0, 400)}`               : null,
  ].filter(Boolean).join('\n')
...
Rules:
- Open with exactly: Hi ${recruiterFirst},
- Keep it under 150 words total
- Tone: ${TONE_DESCRIPTIONS[tone]}
- Reference the company type (${stageLabel}) naturally if it fits
- If industry is provided, tie the candidate's background to it specifically
- If achievements are provided, pick ONE strong data point and use it
- If a job description is provided, tie ONE specific requirement from it to the candidate's background
...
- Subject: 6–9 words, role/company specific, lead with the candidate's strongest signal. The recruiter must decide whether to open it from the subject alone.
